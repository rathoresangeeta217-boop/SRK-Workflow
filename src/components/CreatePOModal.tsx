...
export function CreatePOModal({ isOpen, onClose, products, vendors = [], onCreatePO }: CreatePOModalProps) {
  ...
  const generatePDF = async (product: any, quantity: number, poNumber: string) => {
    ...
    doc.text('Mobile: 6376165128 | Email: Sales@srkmodular.com | Website: https://srkmodular.com/', 105, 32, { align: 'center' });
    ...
    const numericPriceMatch = perUnitPrice.match(/[\d,.]+/);
    ...
    autoTable(doc, {
      ...
      body: [
        [
          '1',
          '', // Image placeholder
          `${product.name}\n${product.specification ? `Size/Spec: ${product.specification}` : ''}`,
