...
const startIndex = code.indexOf('const generateSatisfactionLetter = () => {');
const endIndexStr = 'doc.save(`Satisfaction_Letter_${purchase.id.substring(0,8)}.pdf`);\n  };';
let endIndex = code.indexOf(endIndexStr);

if (startIndex !== -1 && endIndex !== -1) {
  code = code.substring(0, startIndex) + replacement.trim() + code.substring(endIndex + endIndexStr.length);
